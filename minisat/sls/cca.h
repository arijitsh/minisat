...
public:
  ...
  int build_instance(char *filename) {
    ...
    //     free_memory();
    if (verbosity > 1)
      cout << "c [CCAnr] Building Instance" << endl;
    if (verbosity > 1)
      std::cout << "c [CCAnr] Initialized with " << filename << std::endl;
    ...
    if (num_vars >= MAX_VARS || num_clauses >= MAX_CLAUSES) {
      cout << "the size of instance exceeds out limitation, please enlarge "
              "MAX_VARS and (or) MAX_CLAUSES."
           << endl;
      exit(-1);
    }
    ...
    if (simplify == 1) {
      for (c = 0; c < num_clauses; ++c) {
        ...
        for (j = 0; j < org_clause_lit_count[c]; ++j)
          if (cur_soln[org_clause_lit[c][j].var_num] ==
              org_clause_lit[c][j].sense) {
            flag = 1;
            break;
          }
  ...
  */
  ...
  int run(int *soln, int seedp) {
    ...
    if (verbosity > 1)
      std::cout
          << "c This is CCAnr 2.0 [Version: 2018.01.28] [Author: Shaowei Cai]."
          << std::endl;
    times(&start);
    ...
    if (verbosity > 1) {
      ...
      cout << "c Algorithmic: ls_no_improv_steps = " << ls_no_improv_times
           << endl;
    ...
    times(&stop);
    double comp_time = double(stop.tms_utime - start.tms_utime +
    ...
    if (verbosity > 1) {
      cout << "c solveSteps = " << tries << " tries + " << step
           << " steps (each try has " << max_flips << " steps)." << endl;
  ...
  void unit_propagation() {
    ...
    for (unitclause_queue_beg_pointer = 0;
      ...
      for (i = 0; i < var_lit_count[uc_var]; ++i) {
        ...
        if (cur.sense ==
            uc_sense) // then remove the clause from var's var_lit[] array
        {
          clause_delete[c] = 1;
        } else {
          if (clause_lit_count[c] == 2) {
            if (clause_lit[c][0].var_num == uc_var) {
              unitclause_queue[unitclause_queue_end_pointer++] =
                  clause_lit[c][1];
            } else {
              unitclause_queue[unitclause_queue_end_pointer++] =
                  clause_lit[c][0];
